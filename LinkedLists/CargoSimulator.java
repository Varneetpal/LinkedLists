...
public class CargoSimulator {
    ...
    public static void main(String args[]) {
        ...
        while(sim.fleet.itemExists()) {
           // Ship ship = new Ship(sim.fleet.item(), 55);
            System.out.println(sim.fleet.item().name + " has " + sim.fleet.item().sacksOfGrainType(Grain.BARLEY)
                                + " sacks of Barley.");
            sim.fleet.goForth();
        }
        ...
        while(sim.fleet.itemExists()) {
            if (sim.fleet.item().isOverloaded()){
                System.out.println(sim.fleet.item().name + " is overloaded.");
            }
            sim.fleet.goForth();
        }
